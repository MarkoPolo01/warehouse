...
func (r *PostgresRepository) GetFirstFreeSlot(ctx context.Context) (string, error) {
	var slotID string
	err := r.db.QueryRowContext(ctx, 
		"SELECT slot_id FROM slots WHERE is_occupied = false ORDER BY slot_id LIMIT 1").Scan(&slotID)
